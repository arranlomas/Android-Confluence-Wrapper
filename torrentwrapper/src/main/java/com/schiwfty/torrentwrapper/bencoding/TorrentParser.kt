...
object TorrentParser {
    ...
    fun parseTorrent(filePath: String): Observable<ParseTorrentResult> {
        val r = Reader(File(filePath))
        return parseReader(r)
    }

    @Throws(IOException::class)
    fun parseTorrent(byteArray: ByteArray): Observable<ParseTorrentResult> {
        val r = Reader(byteArray)
        return parseReader(r)
    }

    private fun parseReader(r: Reader): Observable<ParseTorrentResult> {
        ...
        if (x.size != 1) ParseTorrentResult.Error(ParseException("Parsing .torrent yielded wrong number of bencoding structs.", 0))
        return parseTorrent(x[0])
    }

    @Throws(ParseException::class)
    private fun parseTorrent(o: Any): Observable<ParseTorrentResult> {
        if (o is BDictionary) {
            ...
            return Observable.just(ParseTorrentResult.Success(t))
        } else {
            return Observable.just(ParseTorrentResult.Error(ParseException("Could not parse Object to BDictionary", 0)))
        }
    }
}
